...
* If all calls to `operator()` return `void`, `fmap` will return `void`. 
* If all instantiated `operator()`s have the same return type, `fmap` returns an array of the return type. 
* If the return values are heterogeneous, a `std::tuple<return_types...>` is returned instead.
