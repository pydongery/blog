...
While we might not be able to provide the desired syntax `foo(3, x=5)`, reflection allows us to inject new class types with named non-static data members. For all intents and purposes the keyword arguments are therefore collected into a named tuple. This means we can constrain the receiving function to express non-optionality of keyword arguments.
```c++
template <typename T>
...
Consider this example ([run on compiler explorer](https://godbolt.org/z/4fYq8aPzc)):
```c++
...
return [:meta::expand(std::ranges::iota_view{0U, pos_only}):]
>> [&]<std::size_t... Idx>{
  ...
  >> [&]<auto... Params> {
    return [:F:](
      ...
      /* keyword arguments */
      get<erl::meta::get_member_index<typename T::type>(identifier_of(Params))>(
        args...[pos_only]
      )...);
  };
};
...
static constexpr void check_args(){
    ...
    >>= [&]<auto Param> {
        ...
        "Positional argument `" + identifier_of(Param) + "` repeated as keyword argument."
        );
    };
...
std::string format_impl(Args const& kwargs) {
    ...
    return [:meta::expand(std::views::iota_view{0U, std::tuple_size_v<Args>}):]
    >> [&]<std::size_t... Idx>() {
