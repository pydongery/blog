...
In this blog post we'll explore implementing order-independent keyword arguments for C++ through use of C++26's [proposed reflection](https://wg21.link/p2996) features. I stumbled across this technique a few days ago while experimenting with reflection and thought it might be worthwhile to share, since it nicely showcases just how powerful the proposed reflection features are.
...
## Prior art
Named, labeled or keyword arguments have been proposed many times throughout the years, but as [EWG issue 150](https://cplusplus.github.io/EWG/ewg-closed.html#150) notes: all of these attempts have failed. Here is several proposals on that topic:
...
Since none of these made it, we have to be somewhat creative to get similar functionality in C++. This also has been done numerous times in various ways, here's a short overview:
...
Usage example ([Run on CE](https://godbolt.org/z/ja5oha875)):
```c++
struct FooArgs {
    int x;
    int y;
...
To accept keyword arguments, functions must wrap their parameters as well - `void foo(int x, int y)` turns into `void foo(TypedArg<int, "x"> x, TypedArg<int, "y"> y)`. Once again, a macro could help here. 

Either way, the wrapped value can then be accessed by using the unary `*` operator on these arguments:
...
```
[Run on CE](https://godbolt.org/z/h7a3vMaqY)
...
The keyword arguments can now be passed in an order-independent way ([Run on CE](https://godbolt.org/z/4ePfobjG5)).
```c++
...
This allows us to write code like this ([Run on CE](https://godbolt.org/z/GP8GE3n8z)):
```c++
...
Usage example ([Run on CE](https://godbolt.org/z/5oPTdsKbz)):
```c++
// optional keyword arguments
...
While we might not be able to provide the desired syntax `foo(3, x=5)`, reflection allows us to inject new class types with named non-static data members. For all intents and purposes the keyword arguments are therefore collected into a named tuple.
...
void foo(kwargs_t<T> const& kwargs) {
...
To work around the lack of member names, we can stringify the capture list and parse it to recover the member names from it. To do this, we finally introduce the macro `make_args`.
```c++
namespace kwargs {
...
Since whitespace is often not significant, we'll need some utility to skip whitespace - let's call it `skip_whitespace`. 
...
We can now write a simple parser for the desired subset of capture-list syntax.
...
For the lambda hackery to work, we need to reflect the lambda's private non-static data members to produce an appropriate keyword argument container. Finally we need to lift them from the lambda produced by the `make_args` macro.
```c++
...
What if we could wrap any function and convert keyword arguments to positional arguments in calls to it as needed? [P3096](https://wg21.link/p3096) function parameter reflection makes this possible.
...
### Implementation
Unfortunately non-trailing packs are not deduced. If our keyword argument tuple were the first this would be trivial, but realistically that does not look very nice and isn't consistent with our previous mixed usage either.

Consider this example ([Run on CE](https://godbolt.org/z/4fYq8aPzc)):
```c++
...
To work around this, we use [P2662](https://wg21.link/p2662) pack indexing to extract the last argument. We can then check if it is a keyword argument container.
```c++
...
To actually combine the received positional arguments with the extracted keyword arguments, we need to expand `args` except for its last element and then retrieve the remaining parameters of `F` from the last element of `args` (which will be the keyword argument container).

> **Expansion**
>
> For the following to work, one adjustment to the aforementioned `expand` helper must be made.
>
> `Replicator`'s `operator>>` expands all elements of the range into the template argument list of a single call to the lambda. Similarly we want `operator>>=` to call the lambda multiple times, with one element as template argument at a time.
>
> ```c++
>template <auto... Vs>
>struct Replicator {
...
>```
>
>Additionally a shorthand `sequence(N)` for `expand(std::ranges::iota_view{0U, N})` shall be introduced. 
>
{: .prompt-tip }
...
Let's also add some diagnostics in case a positional argument was repeated as keyword argument. Similarly we want to provide diagnostics if an argument was missing altogether.
...
Since `std::format` does not yet have support for `fmt`'s named arguments, we can instead transform the format string to something `std::format` is happy with. Since `std::format`'s format string can refer to arguments by position (ie. `std::format("{1}{0}", 0, 42)`), we can transform all named arguments to positional arguments and pass all keyword arguments.

The parser utilities we defined earlier can be re-used for this. Here's an example implementation:
...
### Usage
We can now say `erl::print("{bar} {foo}", make_args(bar=42, foo=123))` instead of something like `std::print("{1} {0}", 123, 42)`. The order in which arguments appear in `make_args` does not matter.

Here's a full example ([Run on compiler explorer](https://cpp26.godbolt.org/z/jh6M7bf6v)):
